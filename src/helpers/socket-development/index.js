$(function () {
    ...
    const riderSockets = ['test', 'tripRequest', 'riderCancel', 'autoCancel', 'cancelTripRequest', 'loginCheck', 'rider', 'tripStatus'];
    const driverSockets = ['test', 'tripDecline', 'tripAccept', 'driverLocation', 'tripStart', 'driverCancel', 'tripComplete', 'loginCheck', 'driver', 'confirmStop', 'tripStatus'];
    ...
    if (userId) {
        if (userType == '1') { // Rider events
            ...
            // Receive - Trip-confirmStop
            socket.on('confirmStop-' + userId, async function (responseData) {
                let html = '<span class="eventName">confirmStop-' + userId + ': </span>';
                html = html + '<span>' + JSON.stringify(responseData) + '</span>';
            ...
            // Receive - Trip-Status
            socket.on('tripStatus-' + userId, async function (responseData) {
                let html = '<span class="eventName">tripStatus-' + userId + ': </span>';
                html = html + '<span>' + JSON.stringify(responseData) + '</span>';
