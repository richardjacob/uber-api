...
export async function pushNotificationMessage(type, requestData, lang) {
	...
	if (type === 'newMessage') {
		if (requestLang == 'en') {
			title = 'Ting-Ting! You got a new message!';
		} else if (requestLang == 'es') {
			title = '¡Ting-Ting! ¡Recibiste un nuevo mensaje!';
		} else if (requestLang == 'fr') {
			title = 'Ting-Ting! Vous avez un nouveau message!';
		} else if (requestLang == 'ru') {
			title = 'Тинг-Тинг! У вас новое сообщение!';
		} else if (requestLang == 'ja') {
			title = 'ティンティン！新しいメッセージが届きました！';
		} else if (requestLang == 'id') {
			title = 'Ting-Ting! Anda mendapat pesan baru!';
		} else if (requestLang == 'hr') {
			title = 'Ting-Ting! Dobili ste novu poruku!';
		} else if (requestLang == 'ar') {
			title = 'تينج تينج! لديك رسالة جديدة!';
		}
	...
	if (type === 'confirmStop') {
		if (requestLang == 'en') {
			title = 'Stop Arrived!';
			message = `${requestData.driverName} has arrived at ${requestData.stopLocation} and will wait for ${requestData.waitingTime} minutes before charges begin for their time`;
		} else if (requestLang == 'es') {
			title = 'Detener llegó!';
			message = `${requestData.driverName} ha llegado a ${requestData.stopLocation} y esperará ${requestData.waitingTime} minutos antes de que comiencen los cargos por su tiempo`;
		} else if (requestLang == 'fr') {
			title = "Arrêtez Arrivé!";
			message = `${requestData.driverName} est arrivé à ${requestData.stopLocation} et attendra ${requestData.waitingTime} minutes avant le début des charges pour leur temps`;
		} else if (requestLang == 'ru') {
			title = 'Стоп прибыл!';
			message = `${requestData.driverName} прибыл в ${requestData.stopLocation} и буду ждать ${requestData.waitingTime} минут до начала зарядки за свое время`;
		} else if (requestLang == 'ja') {
			title = '到着を停止します!';
			message = `${requestData.driverName} に到着しました ${requestData.stopLocation} そして待つでしょう ${requestData.waitingTime} 料金が発生する数分前`;
		} else if (requestLang == 'id') {
			title = 'Berhenti Tiba!';
			message = `${requestData.driverName} telah tiba di ${requestData.stopLocation} dan akan menunggu ${requestData.waitingTime} menit sebelum tagihan dimulai untuk waktunya their`;
		} else if (requestLang == 'hr') {
			title = 'Stop Došao!';
			message = `${requestData.driverName} je stigao u ${requestData.stopLocation} i pričekat će ${requestData.waitingTime} nekoliko minuta prije početka naplate za svoje vrijeme`;
		} else if (requestLang == 'ar') {
			title = "توقف وصل!";
			message = `${requestData.driverName} وصل إلى ${requestData.stopLocation} وسوف ننتظر ${requestData.waitingTime} قبل دقائق من بدء الرسوم لوقتهم`;
		}
	}

	if (type === 'updateStop') {
		if (requestLang == 'en') {
			title = 'The Stop location updated!';
			message = `${requestData.name} has updated the stop location. Please track the ride.`;
		} else if (requestLang == 'es') {
			title = 'Se actualizó la ubicación de la parada!';
			message = `${requestData.name} ha actualizado la ubicación de la parada. Realice un seguimiento del viaje.`;
		} else if (requestLang == 'fr') {
			title = "L'emplacement de l'arrêt mis à jour!";
			message = `${requestData.name} a mis à jour l'emplacement de l'arrêt. Veuillez suivre le trajet.`;
		} else if (requestLang == 'ru') {
			title = 'Место остановки обновлено!';
			message = `${requestData.name} обновил место остановки. Пожалуйста, проследите за поездкой.`;
		} else if (requestLang == 'ja') {
			title = '停止場所が更新されました!';
			message = `${requestData.name} 停止場所を更新しました。乗車を追跡してください。`;
		} else if (requestLang == 'id') {
			title = 'Lokasi Berhenti diperbarui!';
			message = `${requestData.name} telah memperbarui lokasi pemberhentian. Tolong lacak perjalanannya.`;
		} else if (requestLang == 'hr') {
			title = 'Lokacija Stop je ažurirana';
			message = `${requestData.name} je ažurirao lokaciju zaustavljanja. Molimo pratite vožnju.`;
		} else if (requestLang == 'ar') {
			title = "تم تحديث موقع الإيقاف!";
			message = `${requestData.name} قام بتحديث موقع التوقف. الرجاء تتبع الرحلة.`;
		}
	}
