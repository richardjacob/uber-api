...
const createBooking = {
	...
	async resolve({ request }, { requestStatus, data }) {
		let userId, category;
		let pickUpLat, pickUpLng, pickUpLocation, dropOffLat, dropOffLng, dropOffLocation;
		...
		let requestLang, pickUp, dropOff, rideLength;
		...
		try {
			...
			if (requestStatus && requestData) {
				...
				if (requestData.rideDetails && requestData.rideDetails.length > 0) {
					rideLength = requestData.rideDetails && requestData.rideDetails.length;
					pickUp = requestData.rideDetails && requestData.rideDetails[0];
					dropOff = requestData.rideDetails && requestData.rideDetails[rideLength - 1];
					...
					totalDistance = 0;
					totalDuration = 0;
					totalDistance = requestData.rideDetails.reduce((totalDistance, items) => {
						return totalDistance + items.locationDistance
					}, 0);
					totalDuration = requestData.rideDetails.reduce((totalDuration, items) => {
						return totalDuration + items.locationDuration
					}, 0);
				} else {
						...
						if (pricingInfo) {
							...
							if (findNearestDrivers && findNearestDrivers.length > 0) {
								...
								const createTrip = await Booking.create({
									...
									multipleStopsCount: rideLength > 0 ? rideLength - 2 : 0,
									distanceType: requestData.distanceType,
									allowedWaitingTime: requestData.allowedWaitingTime
								});
								...
								let bookingLocationsData = [];
								if (requestData.rideDetails && requestData.rideDetails.length > 0) {
									bookingLocationsData = requestData.rideDetails.map((item, index) => {
										return {
											bookingId,
											...
											locationDistanceType: requestData.distanceType,
								...
								const bookingLocations = await BookingLocations.findAll({
									where: {
										bookingId,
										deletedAt: null
									},
								...
								return await {
									...
									data: {
										...
										riderPayableFare: tripCalculation.riderPayableFare,
										bookingType: 1,
										rideDetails: requestData.rideDetails,
										bookingLocations
									}
								};
							} else {
