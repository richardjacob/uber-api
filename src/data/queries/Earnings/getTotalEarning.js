        ...
        try {
            if (request.user) {
                ...
                let weeklyBookingData = await Booking.findAll({
                    attributes: ['id', 'driverTotalFare', 'currency', 'tripStart', 'tripEnd', 'tipsAmount', 'tollFee'],
                    where: {
                        $and: [
                            { driverId },
...
async function culculateEarnings(bookingData, ratesData, baseCurrency, toCurrency) {
    ...
    if (bookingData && bookingData.length > 0) {
        ...
        await Promise.all(bookingData.map((item) => {
            let bookingFare = 0, tipsFare = 0, tollFee = 0;
            // Calculate duration
            ...
            tollFee = (item.tollFee && Number(item.tollFee) > 0) ? Number(item.tollFee) : 0;
            tollFee = formatExponentialNumber(tollFee);
            if (toCurrency !== item.currency && tollFee > 0) {
                tollFee = convert(baseCurrency, ratesData, tollFee, item.currency, toCurrency);
            }

            totalTips = totalTips + tipsFare; // Tips

            totalEarnings = (totalEarnings + (bookingFare + tipsFare)) - tollFee; // Earnings
        }));
    }
