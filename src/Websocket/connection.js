...
import { confirmStop } from './helpers/confirmStop';
...
const connection = (app, io) => {
	io.on('connection', function (socket) {
		...
		*****/
		...
		/*****
		* The Driver confirm the stop 
		*****/
		socket.on('confirmStop', async function (requestData) {
			let formattedRequest = JSON.parse(requestData);
			let authCheck = socketVerifyJWT_MW(formattedRequest.data);
			let responseData;

			if (authCheck && authCheck.status === 200) {
				responseData = await confirmStop(formattedRequest);

				if (responseData && responseData.status === 200) {
					io.emit('confirmStop-' + responseData.data.riderId, { data: responseData }); // Request to rider for the driver completes the trip
					io.emit('confirmStop-' + responseData.data.driverId, { data: responseData }); // Request to driver for that they complete the trip
				} else {
					io.emit('confirmStop-' + formattedRequest.data.userId, { data: responseData }); // Request to the driver(Error Reponse)
				}
			} else {
				io.emit('confirmStop-' + formattedRequest.data.userId, { // Request to driver(Authentical Fail)
					data: authCheck
				});
			}
		});

		socket.on('tripStatus', async function (requestData) {
			let formattedRequest = JSON.parse(requestData);
			let authCheck = socketVerifyJWT_MW(formattedRequest.data);
			let responseData;

			if (authCheck && authCheck.status === 200) {
				responseData = await tripStatus(formattedRequest);
				if (responseData && responseData.status === 200) {
					io.emit('tripStatus-' + responseData.data.riderId, { data: responseData }); // Request to rider for the driver completes the trip
					io.emit('tripStatus-' + responseData.data.driverId, { data: responseData }); // Request to driver for that they complete the trip
				} else {
					io.emit('tripStatus-' + formattedRequest.data.userId, { data: responseData }); // Request to the driver(Error Reponse)
				}
			} else {
				io.emit('tripStatus-' + formattedRequest.data.userId, { // Request to driver(Authentical Fail)
					data: authCheck
				});
			}
		});
